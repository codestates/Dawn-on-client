          ...
          <LabelContainer>
            {
              subjectLabel.map((ele:any) => 
                ...
                <DeleteBtn onClick={(e:any) => deleteLabel(e)} id={ele}>x</DeleteBtn>
                </div>
              )
            }
          </LabelContainer>
        </div>
        <textarea onChange={(e:any) => setNewData({...newData, todo_comment: e.target.value})} value={newData.todo_comment} className="todobar-todo" placeholder="할 일을 입력해주세요."></textarea>
        <div className="modal-btn-container">
          <button onClick={() => editSave()} className="todobar-save-btn">EDIT</button>
        </div>
      </>
  )
}

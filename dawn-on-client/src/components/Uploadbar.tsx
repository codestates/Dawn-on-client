...
import { changeBackColor, 
        addSelectedTags,
        addToTaglist, 
        addCommentData,
        deleteAtag } from "../module/addTaskModule";
...
function CustomBar () {
  ...
  const data = useSelector((state:RootState) => state.addTaskReducer.plannerDatas);
  ...
  const deleteTag = function (e:any) {
    ...
    if(state.plannerDatas.selected_tags.indexOf(e.target.id)) {
      const deleteTag = state.plannerDatas.selected_tags.filter((el:string) => el !== e.target.id);
      ...
      <CustomContainer>
        ...
        <div className="">
          ...
          <div className="back-color-picker">
            ...
            />
            <span onClick={(e:any) => bgPatternHandler(e)}> <img id="pattern01" style={{borderRadius: "10%"}} alt="pattern" width="24px" src={pattern01} /> </span>
            <span onClick={(e:any) => bgPatternHandler(e)}> <img id="pattern02" style={{borderRadius: "10%"}} alt="pattern" width="24px" src={pattern02} /> </span>
            <span onClick={(e:any) => bgPatternHandler(e)}> <img id="pattern04" style={{borderRadius: "10%"}} alt="pattern" width="24px" src={pattern04} /> </span>
            <span onClick={(e:any) => bgPatternHandler(e)}> <img id="pattern05" style={{borderRadius: "10%"}} alt="pattern" width="24px" src={pattern05} /> </span>
            <span onClick={(e:any) => bgPatternHandler(e)}> <img id="pattern06" style={{borderRadius: "10%"}} alt="pattern" width="24px" src={pattern06} /> </span>
            <span onClick={(e:any) => bgPatternHandler(e)}> <img id="pattern07" style={{borderRadius: "10%"}} alt="pattern" width="24px" src={pattern07} /> </span>
            <span onClick={(e:any) => bgPatternHandler(e)}> <img id="pattern08" style={{borderRadius: "10%"}} alt="pattern" width="24px" src={pattern08} /> </span>
          </div>
        </div>
        <div>
          <h3>Write a comment</h3>
          <textarea 
            onChange={(e:any) => {setComment(e.target.value);dispatch(addCommentData(e.target.value))}} 
        ...
        <div>
          ...
          <div className="select-tag-container">
            { state.tags.map((tag:string) => 
              <div onClick={(e:any) => addToSelected(e)} 
                key={tag} 
                id={tag.toString()}
                className="hashtags-group">#{tag}
                {

                }
                <DeleteBtn key={tag} onClick={(e:any) => deleteTag(e)} id={tag}>x</DeleteBtn>
              </div>
            )}
          </div>
        </div>
